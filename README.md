...
| Cherry-picked **rich formatting** | Bash shows description + command, Edit shows a diff, Write shows line count; each card leads with a state emoji + a markdown subtitle. |
...
- **One status emoji per card** — the title leads with the state: ❓ pending · ✅ approved · ❌ denied · 💬 answered · ⏱️ timed out → CLI · 🏁 turn finished.
- **Notification resolves in place** — after you tap, the pending prompt is *replaced* (via ntfy `sequence_id`): the title emoji flips to the outcome and only the chosen button is kept (`Approved ✔` / `Denied ✖` / the picked option). No "both buttons ticked" confusion; the decision stays in history.
- **Distinguish concurrent sessions**: each title carries a `[project·sessionid]` tag (project = basename of `cwd`, sessionid = first 6 chars of `session_id`), so multiple sessions are told apart. Routing is isolated per request via a random `requestId`; the tag is just visual.
- **AskUserQuestion** — option buttons on your phone (auto-batched into groups of 3, ntfy's action limit; multiple questions asked one card at a time).
- **Plan review** (`ExitPlanMode`) with a longer, separate timeout.
- **Rich previews** in a markdown card (`Claude Code: \`<tool>\`` subtitle + rule + preview): Bash (description + fenced command), Edit (fenced diff), Write (line count), Read/Glob/Grep/WebFetch/WebSearch/Task.
...
```bash
remote-approver init      # generates a random topic, writes config, prints a QR code
                          # interactive: also asks for the ntfy server + optional auth
remote-approver enable    # registers the Claude Code hooks
```

Non-interactive (agent/CI/self-hosted), no prompts:

```bash
remote-approver init --server https://ntfy.example.com --user alex --password '…'
# or --no-input to take all defaults (public ntfy.sh, no auth)
```

(`./install.sh` does init + enable, plus `npm link`.) Scan the QR in the ntfy Android app, then use Claude Code normally. When Claude wants a permission, your phone buzzes with Approve/Deny buttons.
...
- **`timeout`** (default `60`s) — how long to wait for a phone tap before falling back to the terminal prompt. Lower it if you're usually at the keyboard; raise it if usually away.

### Completion notifications (opt-in)

Set `"notifyOnStop": true` to get a one-shot push when Claude finishes a turn — title `[project·sessionid] Claude 干完了`, body = a preview of Claude's last message, no buttons. It fires on the `Stop` hook, which is registered by `enable`, so you only flip the config flag — **no re-running `enable`**. Default is `false` (no completion notifications, no noise). Only the once-per-turn `Stop` event is used; per-tool activity is deliberately not streamed.
...
| `ntfyServer` | `https://ntfy.sh` | ntfy base URL. `init` asks for it interactively, or take it via `--server`; point at your self-hosted instance. |
| `notify` | `true` | Master switch. `false` → keep prompts in the terminal: don't publish or wait on the phone. |
| `timeout` | `60` | Seconds to wait for a phone tap before falling back to the CLI prompt. A compromise between not stalling terminal use and leaving time to tap on the phone. |
...
- To wire a machine **without changing its topic**, pre-write its config then run `remote-approver enable` (registers the hooks, keeps the topic). Use `init` only for first-time provisioning — it regenerates the topic.

## Self-hosted ntfy server (Android, recommended)
...
```bash
docker compose exec ntfy ntfy user add --role=admin alex
docker compose exec ntfy ntfy access alex "cra-*" rw     # allow your approver topics
```
...
```
remote-approver init        # generate topic, write config, print QR (no hook)
remote-approver enable      # register the hooks
remote-approver disable     # remove the hooks (keeps config)
...
remote-approver uninstall   # remove hooks + delete config
```

The hooks are registered under `hooks.PermissionRequest` (approve/deny + AskUserQuestion + plan review) and `hooks.Stop` (completion notification) in `~/.claude/settings.json`. Both run the same `cli.mjs hook` command, which branches on the event. `enable`/`disable` toggle the hooks without touching your topic; `init` (re)generates the topic and writes config.

## What changed vs the base (claude-remote-approver)

- **Rich per-tool previews** (`src/ntfy.mjs`): `formatToolInfo` → `formatToolPreview`, rendered as a markdown card (`subtitle()` + `cardBody()`). The JSON-fallback contract (missing primary field → raw `JSON.stringify(tool_input)`) is preserved.
- **Card format**: title = state emoji + `[project·sessionid]`; markdown body = `Claude Code: \`<tool>\`` subtitle + rule + preview; buttons separate.
- **Resolve-in-place** (`sequence_id`): after a decision the notification is replaced — outcome emoji in the title, only the chosen button kept (`updateNotification` in `src/adapters/claude-code.mjs`).
...
- **Split install**: `init` (config + topic) and `enable` (hook registration) are separate; hooks use the stable `remote-approver` command when installed on `$PATH`.
