...
(`./install.sh` does both, plus `npm link`.) Scan the QR in the ntfy Android app, then use Claude Code normally. When Claude wants a permission, your phone buzzes with Approve/Deny buttons.
...
- **Resolve-in-place** (`sequence_id`): after a decision, the notification is replaced with a button-less `✅/❌/💬/⏱️` outcome card (`updateNotification` in `src/adapters/claude-code.mjs`).
